...
! gfortran MANISCALCO-RMATRIX.f90 ESFINALE-MANISCALCO.f90 -O -llapack -o Exfinale.out
!
! ############################################################################
...
SUBROUTINE TRUE_DURING_TIME(ham0, hamp, bigt, nsteps, NN)
   ...
   ! debugging files
    ...
    OPEN(unit=10, file='true_parity.dat',status='replace')
    ...
    DO step=0, nsteps
        ...
        DO ii=1, num_epsilon
            ...
            parity_vec(ii) = dot_product(hamiltonian%elem(:,ii),matmul(ham_parity%elem,hamiltonian%elem(:,ii)))
        END DO
    
        WRITE(30,*) tt/bigt, epsilon_gs(1:)
        WRITE(80,*) tt/bigt, eigenvalues(1:)
        WRITE(10,*) parity_vec(1:)        
...
PROGRAM MAIN
    ...
    print*, 'What operations do you want do to? ', char(10)
    print*, '1) Only compute true spectrum'
    print*, '2) Only do the time evolution'
    print*, '3) Do both' 
    READ(*,*) decision
    IF(decision==1) THEN
        CALL TRUE_DURING_TIME(ham1, ham2, bigt, nsteps, NN)
    ELSE IF(decision==2) then
        CALL CN_TIME_EVOLUTION(ham1, ham2, gs_start, bigt, nsteps, gs_evol)
    ELSE IF(decision==3) THEN
        CALL CN_TIME_EVOLUTION(ham1, ham2, gs_start, bigt, nsteps, gs_evol)
        CALL TRUE_DURING_TIME(ham1, ham2, bigt, nsteps, NN)
    ELSE
        print*, 'Invalid value, breaking'
        stop
        RETURN
    END IF

    !***** PARITY *****
    CALL PARITY_CHECK(ham1, ham2, NN)
    par =  GS_PARITY(NN)
    print*, 'Ground state parity: ',par
